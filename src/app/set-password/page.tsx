...
function SetPasswordForm() {
  ...
  useEffect(() => {
    if (!token) {
      setError('No invite token found. Please use the full link from your invite.');
    }
  }, [token]);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-md bg-white rounded-lg shadow-lg p-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-2">Set a password to activate your account.</p>
        </div>

        {/* Fatal error (no token) */}
        {!token && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-center">
            <p className="text-red-700 text-sm font-medium">Invalid invite link</p>
            <p className="text-red-600 text-xs mt-1">Please use the full link sent by your admin.</p>
          </div>
        )}
