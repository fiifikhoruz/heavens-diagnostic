 ...
 * Securely downloads a lab report using a time-limited token
  ...
  try {
    ...
    if (!token) {
      return NextResponse.json(
        { error: "Invalid or missing download token" },
        { status: 400 }
      );
    }

    // Validate token - do not mark as used yet (allow multiple downloads)
    const validation = await validateReportToken(token, false);

    if (!validation.valid) {
      return getErrorPage(validation.message || "Invalid or expired token");
    }
    ...
    // Return HTML with appropriate headers for browser display/download
    ...
    return getErrorPage("An unexpected error occurred. Please try again later.");
  }
}

/**
 * Generates an HTML report from visit data
 */
function generateReportHTML(visitData: any): string {
  const patient = visitData.patients;
  const createdBy = visitData.profiles;
        ...
        <div class="patient-info">
          ...
          <div class="info-row">
            ...
            <div class="info-item">
              ...
              <span class="info-value">${formatDate(visitData.visit_date)}</span>
            </div>
          </div>
        </div>
 ...
 */
function getErrorPage(message: string): NextResponse {
  const html = `
    <!DOCTYPE html>
    <html lang="en">
    <head>
